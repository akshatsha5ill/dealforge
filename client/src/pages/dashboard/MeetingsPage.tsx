            ...
            fixedHeaderContent={() => (
              <tr>
                {['Title', 'Date', 'Duration', 'Status'].map((h) => (
                  <th key={h} style={{ backgroundColor: 'var(--bg-secondary)', paddingBottom: '12px', borderBottom: '1px solid var(--border)', fontSize: '12px', fontWeight: 500, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.5px' }}>{h}</th>
                ))}
              </tr>
            )}
            itemContent={(index, m) => (
